...
export default function App() {
  ...
  const fetchAdminUsers = async (page = 1, search = '') => {
    ...
    try {
      const res = await fetch(`/api/admin/users?adminEmail=${user.email}&page=${page}&search=${search}`);
      const data = await res.json();
      if (res.ok) {
                      ...
                      item.type === 'service' ? (
                        ...
                        >
                          ...
                          <div className="space-y-1">
                            ...
                            <h3 className="text-sm font-bold text-gray-900 dark:text-white truncate">{item.data.providerName}</h3>
                      ...
                      {isAdmin && (
                        <div className="flex bg-gray-100 dark:bg-gray-800 p-1 rounded-xl">
                          <button
                            ...
                            className={`px-3 py-1.5 text-[10px] font-bold rounded-lg transition-colors ${adminMainTab === 'users' ? 'bg-white dark:bg-gray-700 text-blue-600 shadow-sm' : 'text-gray-400'}`}
                          >
                            Users
                          </button>
                          <button
                            onClick={() => setAdminMainTab('services')}
                            className={`px-3 py-1.5 text-[10px] font-bold rounded-lg transition-colors ${adminMainTab === 'services' ? 'bg-white dark:bg-gray-700 text-blue-600 shadow-sm' : 'text-gray-400'}`}
                          >
                            Services
                          </button>
                          <button
                            onClick={() => setAdminMainTab('sponsors')}
                            className={`px-3 py-1.5 text-[10px] font-bold rounded-lg transition-colors ${adminMainTab === 'sponsors' ? 'bg-white dark:bg-gray-700 text-blue-600 shadow-sm' : 'text-gray-400'}`}
                          >
                            Sponsor
                          </button>
                        </div>
                    ...
                    {isAdmin && adminMainTab === 'users' && (
                      <div className="space-y-4">
                        ...
                        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 overflow-hidden shadow-sm">
                                ...
                                <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                                  {adminUsers.map((u, i) => (
                                    <tr key={u.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                      <td className="p-3 text-gray-400">{(adminUsersCurrentPage - 1) * 20 + i + 1}</td>
                          ...
                          {adminUsersPages > 1 && (
                            <div className="p-3 border-t border-gray-100 dark:border-gray-700 flex justify-between items-center bg-gray-50/50 dark:bg-gray-700/30">
                              <button
                                disabled={adminUsersCurrentPage === 1}
                                onClick={() => fetchAdminUsers(adminUsersCurrentPage - 1, adminUsersSearch)}
                                className="p-1 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600 disabled:opacity-30 transition-colors"
                              >
                                <ChevronLeft size={18} />
                              </button>
                              <span className="text-[10px] font-bold text-gray-400">Page {adminUsersCurrentPage} of {adminUsersPages}</span>
                              <button
                      ...
                      <div className="space-y-4">
                        <div className="flex bg-gray-100 dark:bg-gray-800 p-1 rounded-xl w-fit">
                          <button
                            ...
                            className={`px-3 py-1.5 text-[10px] font-bold rounded-lg transition-colors ${adminTab === 'pending' ? 'bg-white dark:bg-gray-700 text-blue-600 shadow-sm' : 'text-gray-400'}`}
                          >
                            New
                          </button>
                          <button
                            onClick={() => setAdminTab('approved')}
                            className={`px-3 py-1.5 text-[10px] font-bold rounded-lg transition-colors ${adminTab === 'approved' ? 'bg-white dark:bg-gray-700 text-blue-600 shadow-sm' : 'text-gray-400'}`}
                          >
                            Approved
                          </button>
                        </div>
                      ...
                      <div className="space-y-4">
                        <div className="flex bg-gray-100 dark:bg-gray-800 p-1 rounded-xl w-fit">
                          <button
                            ...
                            className={`px-3 py-1.5 text-[10px] font-bold rounded-lg transition-colors ${sponsorTab === 'new' ? 'bg-white dark:bg-gray-700 text-blue-600 shadow-sm' : 'text-gray-400'}`}
                          >
                            New
                          </button>
                          <button
                            onClick={() => setSponsorTab('old')}
                            className={`px-3 py-1.5 text-[10px] font-bold rounded-lg transition-colors ${sponsorTab === 'old' ? 'bg-white dark:bg-gray-700 text-blue-600 shadow-sm' : 'text-gray-400'}`}
                          >
                            Old
                          </button>
                        </div>
                        {sponsorTab === 'new' ? (
                          <SponsorForm isDarkMode={isDarkMode} onSuccess={fetchSponsors} />
                        ) : (
                          <SponsorList sponsors={sponsors} isDarkMode={isDarkMode} onUpdate={fetchSponsors} />
                        )}
                      </div>
                    )}
  ...
  return (
    <div className="bg-white dark:bg-gray-800 p-3 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 flex flex-col gap-2 relative overflow-hidden h-full min-h-[160px]">
      ...
      <div className="mt-auto relative z-10 p-1">
        ...
        <p className="text-[9px] text-gray-300 truncate">{sponsor.phone}</p>
      </div>
    </div>
  );
}

function SponsorForm({ isDarkMode, onSuccess }: { isDarkMode: boolean, onSuccess: () => void }) {
...
const handleSubmit = async (e: React.FormEvent) => {
  ...
  // FIX: Manually get the user from storage so we don't rely on a variable name
  ...
  try {
    const res = await fetch('/api/sponsors', {
      ...
      body: JSON.stringify({ 
        ...formData, 
        adminEmail: storedUser.email, // Use the email from storage
        photoUrls: photos 
      })
    });
...
function SponsorList({ sponsors, isDarkMode, onUpdate }: { sponsors: Sponsor[], isDarkMode: boolean, onUpdate: () => void }) {
  ...
  return (
    <div className="space-y-3">
      {sponsors.length > 0 ? (
        sponsors.map(s => (
          <div key={s.id} className="bg-white dark:bg-gray-800 p-4 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm space-y-3">
            <div className="flex justify-between items-start">
              <div>
                <h4 className="font-bold text-sm">{s.name}</h4>
                <p className="text-[10px] text-gray-500">{s.email} | {s.phone}</p>
              </div>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => toggleSponsor(s.id, !s.isEnabled)}
                  className={`w-10 h-5 rounded-full transition-colors relative ${s.isEnabled ? 'bg-green-500' : 'bg-gray-300'}`}
                >
                  <div className={`absolute top-0.5 w-4 h-4 rounded-full bg-white transition-all ${s.isEnabled ? 'left-5.5' : 'left-0.5'}`} />
                </button>
                <button onClick={() => deleteSponsor(s.id)} className="text-red-500 p-1 hover:bg-red-50 rounded-lg">
                  <Trash2 size={16} />
                </button>
              </div>
            </div>
            {s.isEnabled && s.photoUrls.length > 0 && (
              <div className="flex gap-2 overflow-x-auto pb-1">
                {s.photoUrls.map((url, i) => (
                  <img key={i} src={url} className="w-20 h-20 rounded-xl object-cover border border-gray-100" />
                ))}
              </div>
            )}
          </div>
        ...
        {mode === 'signup' && (
          <div className="flex gap-2">
            <div className="relative w-32">
              <select
                className={`w-full p-3 pl-8 rounded-xl text-sm outline-none focus:ring-1 focus:ring-blue-500 appearance-none transition-colors ${isDarkMode ? 'bg-gray-200 text-gray-900' : 'bg-gray-50 text-gray-900'}`}
                value={countryCode}
              ...
              <Globe className="absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400" size={16} />
            </div>
            <input
              type="tel"
              placeholder="Mobile Number (e.g. 012-3456789)"
              required
              className={`flex-1 p-3 rounded-xl text-sm outline-none focus:ring-1 focus:ring-blue-500 transition-colors ${isDarkMode ? 'bg-gray-200 text-gray-900 placeholder-gray-500' : 'bg-gray-50 text-gray-900'}`}
      ...
      >
        <div className="flex justify-between items-center mb-6">
          ...
          <button onClick={onClose} className={`p-2 rounded-full transition-colors ${isDarkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-100'}`}>
            <X size={20} className={isDarkMode ? 'text-black' : 'text-gray-500'} />
          </button>
        </div>
